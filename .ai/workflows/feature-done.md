# Procédure interne — feature-done
...
**Procedure chain** : `.ai/workflows/feature-update.md` (phase=review) → **`.ai/workflows/quality-gate.md`** → **`.ai/workflows/feature-done.md`** → commit.
...
### Phase 1 — Pré-requis
1. Vérifier que `progress.phase` ∈ {`review`, `done`}. Sinon → STOP, demander à l'utilisateur de passer par `feature-update` d'abord.
2. Vérifier que `progress.blockers` est vide. Sinon → STOP.
3. Lire la fiche et le worklog en entier.

### Phase 2 — Quality gate
Appliquer `.ai/workflows/quality-gate.md` avec `<scope/id>`. Verdict NO-GO → STOP, transférer les actions à l'utilisateur.
