...
- `.ai/index.md` (séquence canonique — pour vérifier que `guardrails.md` reste on-demand)
...
### Phase 6 — Référence on-demand

Vérifier que `.ai/index.md` ne force pas `guardrails.md` dans Pack A.
- **Mention dans Pack A** → proposer de la retirer et de garder `guardrails.md` on-demand.
- **Aucune mention** → rien à faire.
- **Besoin d'un pointeur** → proposer une ligne hors Pack A, section On Demand :
  ```
  - `.ai/guardrails.md` : charger seulement pour cadrage produit, non-goals ou glossaire métier.
  ```
...
✅ guardrails conservé hors Pack A
...
- **Pas d'injection runtime** : ne pas modifier `.ai/scripts/pre-turn-reminder.sh` ni `.ai/reminder.md`. `guardrails.md` est on-demand, pas Pack A.
